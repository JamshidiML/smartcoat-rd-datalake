...
class SDKAndAuthorityBoundaryTests(unittest.TestCase):
    ...
    def test_http_mediator_requires_and_sends_bearer_token(self) -> None:
        class Response:
            def __enter__(self):
                return self
            ...
            def read(self):
                return b'{"legal_hold":"ON"}'

        with patch.object(enforcement.urllib.request, "urlopen", return_value=Response()) as opened:
            mediator = enforcement.HttpLegalHoldMediator(
                "http://legal-hold-applier:8090", "t" * 32
            )
            self.assertEqual("ON", mediator.apply_on(target()))
        request = opened.call_args.args[0]
        self.assertEqual("Bearer " + "t" * 32, request.get_header("Authorization"))
        with self.assertRaises(enforcement.RetentionEnforcementError) as caught:
            enforcement.HttpLegalHoldMediator(
                "http://legal-hold-applier:8090", "too-short"
            )
        self.assertEqual("LEGAL_HOLD_CALL_TOKEN_INVALID", caught.exception.code)

    def test_compose_injects_url_and_call_token_but_not_minio_credentials_into_api(self) -> None:
        compose = (ROOT / "compose.yaml").read_text()
        start = compose.index("  api:")
        end = compose.index("\n  web:", start)
        api = compose[start:end]
        self.assertIn("LEGAL_HOLD_APPLIER_URL: http://legal-hold-applier:8090", api)
        self.assertIn("LEGAL_HOLD_APPLIER_CALL_TOKEN", api)
