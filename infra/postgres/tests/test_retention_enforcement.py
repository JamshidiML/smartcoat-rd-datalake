...
class RetentionEnforcementMigrationTests(unittest.TestCase):
    ...
    def test_collision_free_migration_identity_and_full_intended_chain_slot(self) -> None:
        ...
        self.assertEqual([1, 5, 7], [item.version for item in discovered])
    ...
    def test_runtime_has_insert_and_read_only_not_mutation_or_ddl(self) -> None:
        grants = re.findall(r"GRANT\s+([^;]+);", self.sql, re.IGNORECASE)
        self.assertEqual(
            ["SELECT, INSERT ON bronze_retention_enforcement_evidence TO smartcoat_app"],
            [" ".join(value.split()) for value in grants],
        )
