...
pulls or builds an image, and labels every generated Docker resource for
ownership-validated cleanup.  Four independent projects cover fresh and
upgraded success, a protected-original orphan, and a protected-pair database
failure followed by idempotent reconciliation.
"""
...
API_IMAGE = "smartcoat-rd-datalake-api:latest"
...
class Scenario:
    def __init__(self, name: str, image_ids: dict[str, str]) -> None:
        ...
        self.secrets = {
            ...
            *self.app.values(), *self.mediator.values(),
        }
    ...
    def run_api_python(
        ...
        *,
        ...
        command = [
            ...
            "--mount", f"type=bind,src={ROOT / 'apps/api/src'},dst=/candidate,readonly",
        ]
        for name in environment:
            command.extend(["--env", name])
    ...
    def execute(self) -> None:
        ...
        if self.name == "manifest_failure":
            ...
            return
        if self.name == "transaction_failure_reconciliation":
            psql(
                self.pg_container, self.admin, self.database,
            ...
            result = json.loads(
                self.run_api_python(
                    INGEST_PROGRAM, self.ingestion_environment("transaction_failure")
                ).stdout
            )
            ...
            if not (
                ...
                == {item["version_id"] for item in targets}
            ):
